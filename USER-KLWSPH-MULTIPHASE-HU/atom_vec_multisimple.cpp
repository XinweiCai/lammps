...
//// 这个就是从line改过来，把名字换了，把2d条件去了

#include "atom_vec_multisimple.h"
#include <cmath>
#include <cstring>
#include "atom.h"
#include "comm.h"
...
AtomVecMultisimple::AtomVecMultisimple(LAMMPS *lmp) : AtomVec(lmp)
{
  molecular = 0;
...
AtomVecMultisimple::~AtomVecMultisimple()
{
  memory->sfree(bonus);
}
...
void AtomVecMultisimple::grow(int n)
{
  if (n == 0) grow_nmax();
  else nmax = n;
  atom->nmax = nmax;
...
void AtomVecMultisimple::grow_reset()
{
...
void AtomVecMultisimple::grow_bonus()
{
...
void AtomVecMultisimple::copy(int i, int j, int delflag)
{
  tag[j] = tag[i];
  type[j] = type[i];
  mask[j] = mask[i];
...
void AtomVecMultisimple::copy_bonus(int i, int j)
{
...
void AtomVecMultisimple::clear_bonus()
{
  nghost_bonus = 0;
...
void AtomVecMultisimple::set_length(int i, double value)
{
  if (line[i] < 0) {
    if (value == 0.0) return;
...
int AtomVecMultisimple::pack_comm(int n, int *list, double *buf,
...
int AtomVecMultisimple::pack_comm_vel(int n, int *list, double *buf,
                               int pbc_flag, int *pbc)
{
  int i,j,m;
...
int AtomVecMultisimple::pack_comm_hybrid(int n, int *list, double *buf)
{
  int i,j,m;
...
void AtomVecMultisimple::unpack_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecMultisimple::unpack_comm_vel(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecMultisimple::unpack_comm_hybrid(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecMultisimple::pack_reverse(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecMultisimple::pack_reverse_hybrid(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecMultisimple::unpack_reverse(int n, int *list, double *buf)
{
  int i,j,m;
...
int AtomVecMultisimple::unpack_reverse_hybrid(int n, int *list, double *buf)
{
  int i,j,m;
...
int AtomVecMultisimple::pack_border(int n, int *list, double *buf,
                             int pbc_flag, int *pbc)
{
  int i,j,m;
  double dx,dy,dz;
...
int AtomVecMultisimple::pack_border_vel(int n, int *list, double *buf,
                                 int pbc_flag, int *pbc)
{
  int i,j,m;
  double dx,dy,dz,dvx,dvy,dvz;
...
int AtomVecMultisimple::pack_border_hybrid(int n, int *list, double *buf)
{
  int i,j,m;
...
void AtomVecMultisimple::unpack_border(int n, int first, double *buf)
{
  int i,j,m,last;
...
void AtomVecMultisimple::unpack_border_vel(int n, int first, double *buf)
{
  int i,j,m,last;
...
int AtomVecMultisimple::unpack_border_hybrid(int n, int first, double *buf)
{
  int i,j,m,last;
...
int AtomVecMultisimple::pack_exchange(int i, double *buf)
{
  int m = 1;
  buf[m++] = x[i][0];
  buf[m++] = x[i][1];
...
int AtomVecMultisimple::unpack_exchange(double *buf)
{
...
int AtomVecMultisimple::size_restart()
{
  int i;
...
int AtomVecMultisimple::pack_restart(int i, double *buf)
{
  int m = 1;
  buf[m++] = x[i][0];
  buf[m++] = x[i][1];
...
int AtomVecMultisimple::unpack_restart(double *buf)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) {
    grow(0);
...
void AtomVecMultisimple::create_atom(int itype, double *coord)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
...
void AtomVecMultisimple::data_atom(double *coord, imageint imagetmp, char **values)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
...
int AtomVecMultisimple::data_atom_hybrid(int nlocal, char **values)
{
...
void AtomVecMultisimple::data_atom_bonus(int m, char **values)
{
...
void AtomVecMultisimple::data_vel(int m, char **values)
{
...
int AtomVecMultisimple::data_vel_hybrid(int m, char **values)
{
...
void AtomVecMultisimple::pack_data(double **buf)
{
  int nlocal = atom->nlocal;
...
int AtomVecMultisimple::pack_data_hybrid(int i, double *buf)
{
...
void AtomVecMultisimple::write_data(FILE *fp, int n, double **buf)
{
  for (int i = 0; i < n; i++)
...
int AtomVecMultisimple::write_data_hybrid(FILE *fp, double *buf)
{
...
void AtomVecMultisimple::pack_vel(double **buf)
{
  int nlocal = atom->nlocal;
  for (int i = 0; i < nlocal; i++) {
    buf[i][0] = ubuf(tag[i]).d;
...
int AtomVecMultisimple::pack_vel_hybrid(int i, double *buf)
{
  buf[0] = omega[i][0];
  buf[1] = omega[i][1];
  buf[2] = omega[i][2];
...
void AtomVecMultisimple::write_vel(FILE *fp, int n, double **buf)
{
  for (int i = 0; i < n; i++)
...
int AtomVecMultisimple::write_vel_hybrid(FILE *fp, double *buf)
{
...
bigint AtomVecMultisimple::memory_usage()
{
  bigint bytes = 0;
...
/*
void AtomVecMultisimple::consistency_check(int n, char *str)
{
  int iflag = 0;
  int count = 0;
  for (int i = 0; i < n; i++) {
