...
AtomVecMeso::AtomVecMeso(LAMMPS *lmp) : AtomVec(lmp)
{
  ...
  size_forward = 14; // 3 + rho + e + vest[3] + vv[3] + v[3], that means we may only communicate 5 in hybrid
  size_reverse = 8; // 3+3 + drho + de
  size_border = 18; // 6 + rho + e + vest[3] + cv + vv[3] + v[3]
  size_velocity = 3;
  size_data_atom = 8;
  size_data_vel = 4;
  xcol_data = 6;
...
void AtomVecMeso::grow(int n)
{
  ...
  vv = memory->grow(atom->vv, nmax, 3, "atom:vv");
  fpb = memory->grow(atom->fpb, nmax*comm->nthreads, 3, "atom:fpb");
...
int AtomVecMeso::size_restart() {
  ...
  int nlocal = atom->nlocal;
  int n = 23 * nlocal; // 11 + rho + e + cv + vest[3] + vv[3] + v[3]
...
bigint AtomVecMeso::memory_usage() {
  ...
  if (atom->memcheck("vv"))
	  bytes += memory->usage(vv, nmax);
  if (atom->memcheck("fpb"))
	  bytes += memory->usage(fpb, nmax*comm->nthreads, 3);
  return bytes;
}
