...
#include <math.h>
#include <stdlib.h>
#include "pair_sph_morris1997_noslip_zflat.h"
#include "atom.h"
#include "force.h"
#include "comm.h"
...
PairSPHMorris1997NoslipZFlat::PairSPHMorris1997NoslipZFlat(LAMMPS *lmp) : Pair(lmp)
{
  restartinfo = 0;
  first = 1;
}
...
PairSPHMorris1997NoslipZFlat::~PairSPHMorris1997NoslipZFlat() {
  if (allocated) {
...
inline vector<double, 3> noslip(const double zs, const vector<double, 3> &xb, const vector<double, 3> &xf,
                                const vector<double, 3> &vb, const vector<double, 3> &vf, const double beta, const vector<double, 3> &vs) {
  const double df = xf[2] - zs; // 流体粒子到边界的距离
  const double db = xb[2] - zs; // 固体粒子到边界的距离
  double scale =  - db / df;
  if (scale < 0)
    return vb;
...
void PairSPHMorris1997NoslipZFlat::compute(int eflag, int vflag) {
...
void PairSPHMorris1997NoslipZFlat::allocate() {
  allocated = 1;
  int n = atom->ntypes;
...
void PairSPHMorris1997NoslipZFlat::settings(int narg, char **arg) {
  if (narg != 0)
    error->all(FLERR,
...
void PairSPHMorris1997NoslipZFlat::coeff(int narg, char **arg) {
  if (narg != 10)
    error->all(FLERR,
  ...
  double vsurface_one = force->numeric(FLERR, arg[2]); //the velocity of flat.
  ...
  double zflat_one = force->numeric(FLERR,arg[8]); //The height of the flat in the z direction
...
double PairSPHMorris1997NoslipZFlat::init_one(int i, int j) {
...
double PairSPHMorris1997NoslipZFlat::single(int i, int j, int itype, int jtype,
