...
#include <math.h>
#include <stdlib.h>
#include "pair_sph_angmom_noslip_flat.h"
#include "atom.h"
#include "force.h"
#include "comm.h"
...
PairSPHAngmomNoslipFlat::PairSPHAngmomNoslipFlat(LAMMPS *lmp) : Pair(lmp)
{
  restartinfo = 0;
  first = 1;
}
...
PairSPHAngmomNoslipFlat::~PairSPHAngmomNoslipFlat() {
  if (allocated) {
...
void PairSPHAngmomNoslipFlat::compute(int eflag, int vflag) {
...
*/
       ...
       // printf("testvar= %f, %f \n", delx, dely);
        ...
        f[i][0] += delx * fpair + delx * fvisc;
        f[i][1] += dely * fpair + dely * fvisc;
        f[i][2] += delz * fpair + delz * fvisc;
        ...
        if (newton_pair || j < nlocal) {
          ...
          f[j][0] -= delx * fpair + delx * fvisc;
          f[j][1] -= dely * fpair + dely * fvisc;
          f[j][2] -= delz * fpair + delz * fvisc;
          de[j] += deltaE;
...
void PairSPHAngmomNoslipFlat::allocate() {
  allocated = 1;
  int n = atom->ntypes;
...
void PairSPHAngmomNoslipFlat::settings(int narg, char **arg) {
  if (narg != 0)
    error->all(FLERR,
...
void PairSPHAngmomNoslipFlat::coeff(int narg, char **arg) {
  if (narg != 11)
    error->all(FLERR,
  ...
  double vsurface_one = force->numeric(FLERR, arg[10]); //the v of flat.
...
double PairSPHAngmomNoslipFlat::init_one(int i, int j) {
...
double PairSPHAngmomNoslipFlat::single(int i, int j, int itype, int jtype,
