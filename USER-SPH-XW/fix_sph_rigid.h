...
FixStyle(sph/rigid,FixSPHRigid)

#else

#ifndef LMP_FIX_SPH_RIGID_H
#define LMP_FIX_SPH_RIGID_H

#include "fix.h"

namespace LAMMPS_NS {

class FixSPHRigid : public Fix {
 public:
  FixSPHRigid(class LAMMPS *, int, char **);
  virtual ~FixSPHRigid();
  virtual int setmask();
  virtual void init();
 ...
 protected:
  ...
  double **vcmest;             // velocity of center-of-mass of each
  double **angmomest;          // angular momentum of each in space coords
  double **omegaest;           // angular velocity of each in space coords
