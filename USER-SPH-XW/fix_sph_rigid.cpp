...
FixSPHRigid::FixSPHRigid(LAMMPS *lmp, int narg, char **arg) :
  ...
  avec_line(NULL), avec_tri(NULL)
{
  ...
  memory->create(vcmest,nbody,3,"rigid:vcmest");
  memory->create(angmomest,nbody,3,"rigid:angmomest");
  memory->create(omegaest,nbody,3,"rigid:omegaest");
...
FixSPHRigid::~FixSPHRigid()
{
  ...
  memory->destroy(vcmest);
  memory->destroy(angmomest);
  memory->destroy(omegaest);
...
void FixSPHRigid::setup_pre_neighbor()
{
  pre_neighbor();
}
...
void FixSPHRigid::setup(int vflag)
{
  int i,n,ibody;
...
void FixSPHRigid::setup_pre_force(int vflag)
{
  // set vest equal to v
  double **v = atom->v;
...
void FixSPHRigid::initial_integrate(int vflag)
{
  double dtfm;
  ...
  for (int ibody = 0; ibody < nbody; ibody++) {
    ...
    vcmest[ibody][0] = vcm[ibody][0] + dtfm * fcm[ibody][0] * fflag[ibody][0];
    vcmest[ibody][1] = vcm[ibody][1] + dtfm * fcm[ibody][1] * fflag[ibody][1];
    vcmest[ibody][2] = vcm[ibody][2] + dtfm * fcm[ibody][2] * fflag[ibody][2];
    ...
    angmomest[ibody][0] = angmom[ibody][0] + dtf * torque[ibody][0] * tflag[ibody][0];
    angmomest[ibody][1] = angmom[ibody][1] + dtf * torque[ibody][1] * tflag[ibody][1];
    angmomest[ibody][2] = angmom[ibody][2] + dtf * torque[ibody][2] * tflag[ibody][2];
    ...
    MathExtra::angmom_to_omega(angmom[ibody],ex_space[ibody],ey_space[ibody],
                               ez_space[ibody],inertia[ibody],omega[ibody]);
    MathExtra::angmom_to_omega(angmomest[ibody],ex_space[ibody],ey_space[ibody],
                               ez_space[ibody],inertia[ibody],omegaest[ibody]);
...
void FixSPHRigid::post_force(int vflag)
{
  if (me == 0) {
...
void FixSPHRigid::final_integrate()
{
  int i,ibody;
  double dtfm;
...
void FixSPHRigid::initial_integrate_respa(int vflag, int ilevel, int iloop)
{
...
void FixSPHRigid::final_integrate_respa(int ilevel, int iloop)
{
  dtf = 0.5 * step_respa[ilevel] * force->ftm2v;
  final_integrate();
}
...
int FixSPHRigid::dof(int tgroup)
{
...
void FixSPHRigid::deform(int flag)
{
  if (flag == 0)
    for (int ibody = 0; ibody < nbody; ibody++)
...
void FixSPHRigid::set_xv()
{
  ...
  for (int i = 0; i < nlocal; i++) {
    ...
    vest[i][0] = omegaest[ibody][1]*x[i][2] - omegaest[ibody][2]*x[i][1] +
      vcmest[ibody][0];
    vest[i][1] = omegaest[ibody][2]*x[i][0] - omegaest[ibody][0]*x[i][2] +
      vcmest[ibody][1];
    vest[i][2] = omegaest[ibody][0]*x[i][1] - omegaest[ibody][1]*x[i][0] +
      vcmest[ibody][2];
...
void FixSPHRigid::setup_bodies_static()
{
  int i,ibody;
...
void FixSPHRigid::setup_bodies_dynamic()
{
  int i,ibody;
...
void FixSPHRigid::readfile(int which, double *vec,
...
void FixSPHRigid::write_restart_file(char *file)
{
  if (me) return;
...
double FixSPHRigid::memory_usage()
{
  int nmax = atom->nmax;
...
void FixSPHRigid::grow_arrays(int nmax)
{
...
void FixSPHRigid::copy_arrays(int i, int j, int delflag)
{
  body[j] = body[i];
...
void FixSPHRigid::set_arrays(int i)
{
  body[i] = -1;
  xcmimage[i] = 0;
  displace[i][0] = 0.0;
...
int FixSPHRigid::pack_exchange(int i, double *buf)
{
  buf[0] = ubuf(body[i]).d;
...
int FixSPHRigid::unpack_exchange(int nlocal, double *buf)
{
...
void FixSPHRigid::zero_momentum()
{
  for (int ibody = 0; ibody < nbody; ibody++)
    vcm[ibody][0] = vcm[ibody][1] = vcm[ibody][2] = 0.0;
...
void FixSPHRigid::zero_rotation()
{
  for (int ibody = 0; ibody < nbody; ibody++) {
    angmom[ibody][0] = angmom[ibody][1] = angmom[ibody][2] = 0.0;
...
double FixSPHRigid::compute_scalar()
{
...
void *FixSPHRigid::extract(const char *str, int &dim)
{
...
double FixSPHRigid::extract_erotational()
{
  double wbody[3],rot[3][3];
...
double FixSPHRigid::compute_array(int i, int j)
{
  ...
  if (j == 14) (imagebody[i] >> IMG2BITS) - IMGMAX;
  return omega[i][j-15];
}
