...
#include <math.h>
#include <stdlib.h>
#include "pair_sph_adami_noslip_boundary.h"
#include "atom.h"
#include "force.h"
#include "comm.h"
...
PairSPHAdamiNoslipBoundary::PairSPHAdamiNoslipBoundary(LAMMPS *lmp) : Pair(lmp)
{
	restartinfo = 0;
	first = 1;
}
...
PairSPHAdamiNoslipBoundary::~PairSPHAdamiNoslipBoundary() {
	if (allocated) {
...
void PairSPHAdamiNoslipBoundary::compute(int eflag, int vflag) {
	...
	double rsq, tmp, wfd, sumwf, delVdotDelR, deltaE;
...
void PairSPHAdamiNoslipBoundary::allocate() {
	allocated = 1;
	int n = atom->ntypes;
...
void PairSPHAdamiNoslipBoundary::settings(int narg, char **arg) {
	int rigid_group;
	if (narg != 1)
		error->all(FLERR,
...
void PairSPHAdamiNoslipBoundary::coeff(int narg, char **arg) {
	if (narg != 7)
		error->all(FLERR,
...
double PairSPHAdamiNoslipBoundary::init_one(int i, int j) {
...
double PairSPHAdamiNoslipBoundary::single(int i, int j, int itype, int jtype,
