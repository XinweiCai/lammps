...
#include "pair_sph_wkl_rhosumi.h"
#include "atom.h"
#include "force.h"
#include "comm.h"
...
PairSPHWklRhoSumI::PairSPHWklRhoSumI(LAMMPS *lmp) : Pair(lmp)
{
  restartinfo = 0;
...
PairSPHWklRhoSumI::~PairSPHWklRhoSumI() {
  if (allocated) {
...
void PairSPHWklRhoSumI::init_style() {
...
void PairSPHWklRhoSumI::compute(int eflag, int vflag) {
...
void PairSPHWklRhoSumI::allocate() {
  allocated = 1;
  int n = atom->ntypes;
...
void PairSPHWklRhoSumI::settings(int narg, char **arg) {
  if (narg != 1)
    error->all(FLERR,
...
void PairSPHWklRhoSumI::coeff(int narg, char **arg) {
  if (narg != 3)
...
double PairSPHWklRhoSumI::init_one(int i, int j) {
  if (setflag[i][j] == 0) {
...
double PairSPHWklRhoSumI::single(int /*i*/, int /*j*/, int /*itype*/, int /*jtype*/, double /*rsq*/,
...
int PairSPHWklRhoSumI::pack_forward_comm(int n, int *list, double *buf,
...
void PairSPHWklRhoSumI::unpack_forward_comm(int n, int first, double *buf) {
  int i, m, last;
  double *rho = atom->rho;
